// practica10.cpp : Этот файл содержит функцию "main". Здесь начинается и заканчивается выполнение программы.
//
...
DWORD WINAPI ThreadFunction1(LPVOID lpParam) {
	...
	for (int i = 0; i < 10; ++i) {
		std::cout << "Поток 1, значение: " << pow(i,2) << std::endl;
	}
	std::cout << "Поток 1 завершил вычисления.\n";
	return 0;
}
...
DWORD WINAPI ThreadFunction2(LPVOID lpParam) {
	...
	std::string s;
	std::cout << "Введите строку в поток 2 для записи в файл: \n";
	std::cin >> s;
	...
	if (file.is_open()) {
		file << s;
		file.close();
		std::cout << "Поток 2 записал ввод в файл 'output.txt'.\n";
	}
	else {
		std::cerr << "Поток 2 не смог открыть файл для записи.\n";
	}
...
int main() {
	...
	// Получение псевдодескриптора текущего процесса (инициализация)
	...
	wsprintf(lpszComLine, L"C:\\Users\\ivang\\OneDrive\\Рабочий стол\\college_3_kurs\\SistProg\\practica10\\x64\\Debug\\practica10_addition.exe %d", (int)hInheritProcess);
	...
	// Записываю строку в файл через дубликат дескриптора
	char buffer[1024] = "Привет, мир!";
	DWORD dwWritten;
